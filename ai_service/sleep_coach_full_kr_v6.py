...
def chat(uid: str, sys: str, user: str) -> tuple[str, int, int]:
    ...
    gpt-oss 같은 reasoning 모델은 *reasoning_content* + *content* 두 필드를 분리해 반환.
    max_tokens가 작으면 reasoning에 다 소비돼 content가 None인 경우가 있어, fallback으로
    reasoning_content를 사용하고 max_tokens도 충분히 크게 둔다.
    """
    resp = client.chat.completions.create(
        model=LLM_MODEL,
        temperature=0.3,
        top_p=0.30,
        max_tokens=2048,   # reasoning model 대응 — reasoning + content 모두 충분
        messages=[
            {"role": "system", "content": sys},
            {"role": "user",   "content": user},
        ],
    ...
    content = msg.content
    if not content:
        # reasoning model에서 max_tokens가 reasoning에 다 소비되면 content가 None
...
# ─────────────────────────────────────────────────────────
# Phase D 운영 통합 — PSQI 4 모델 + Forward Simulation 추천 (옵트인)
# USE_PSQI_FORWARD_SIM=1일 때만 활성. 실패 시 모든 키 None — 기존 단일 모델 path가 그대로 작동.
# ─────────────────────────────────────────────────────────
def _compute_psqi_block(uid: str, train_master: pd.DataFrame) -> dict:
    """4 차원 PSQI 점수·4 모델·추천 슬롯·LLM 프롬프트 block을 산출.

    Returns dict with keys:
        psqi_scores: {c1, c2, c3, c4, total} 또는 None
        psqi_predicted_total: float 또는 None (추천 슬롯의 예측 합산)
        recommended_slot: dict 또는 None
        recommendation_mode: 'exploration' / 'exploitation' / None
        prompt_block: str (LLM prompt 끝에 append할 PSQI 정보)
    """
    out = {
        "psqi_scores":          None,
        "psqi_predicted_total": None,
    ...
    try:
        ...
        if "/" not in _sys.path:
            _sys.path.insert(0, "/")
        from experiments.lib import psqi as P_psqi
        from experiments.lib import models as P_models
        from experiments.lib import recommend_psqi as P_rec
        from experiments.lib import exploration_phase as P_exp

        # 1) PSQI target — DB에서 sleep_summary·sleep_detail·feedback·bedtime_log 조회
        daily = read_table(f"{uid}_sleep_summary", where=f"user_id = '{uid}'")
        sleep_detail = read_table(f"{uid}_sleep_detail", where=f"user_id = '{uid}'")
        try:
            feedback = read_table(f"{uid}_feedback", where=f"user_id = '{uid}'")
        except Exception:
            feedback = None
        try:
            bedtime_log = read_table("bedtime_log", where=f"user_id = '{uid}'")
        except Exception:
            bedtime_log = None

        psqi_df = P_psqi.compute_psqi_scores(
            daily, feedback=feedback, sleep_detail=sleep_detail, bedtime_log=bedtime_log,
        )
        psqi_df["date"] = pd.to_datetime(psqi_df["date"])

        # 2) train_master에 PSQI target merge (4 차원만)
        target_cols = list(P_models.PSQI_DIMS)
        tm = train_master.copy()
        tm["date"] = pd.to_datetime(tm["date"])
        train_with_psqi = tm.merge(
            psqi_df[["user_id", "date"] + target_cols],
            on=["user_id", "date"], how="left",
        )

        # 3) 현재(가장 최근 일자) 4 차원 점수
        current_row = train_with_psqi.iloc[-1]
        current = {}
        for k_in, k_out in [("c1_subjective", "c1"), ("c2_latency", "c2"),
                            ("c3_duration", "c3"), ("c4_efficiency", "c4")]:
            v = current_row.get(k_in)
            current[k_out] = float(v) if pd.notna(v) else None
        non_null = [v for v in current.values() if v is not None]
        current["total"] = float(sum(non_null)) if non_null else None
        out["psqi_scores"] = current

        # 4) 추천 모드 결정
        mode, slot_info = P_exp.recommendation_mode(train_with_psqi)
        out["recommendation_mode"] = mode

        if mode == "exploitation":
            # 5) 4 모델 학습 — features는 numeric & target/key 제외
            non_features = set(target_cols) | {"user_id", "date", TARGET}
            feature_cols = [c for c in train_with_psqi.columns
                            if c not in non_features
                            and pd.api.types.is_numeric_dtype(train_with_psqi[c])]

            _results, dim_models = P_models.train_eval_psqi_4dim(
                train_with_psqi, feature_cols=feature_cols,
                dims=target_cols, holdout_days=7, iterations=300,
            )

            if len(dim_models) == len(target_cols):
                # 6) Forward simulation 추천
                activity_1min = read_table(f"{uid}_activity_1min", where=f"user_id = '{uid}'")
                recs = P_rec.recommend_psqi_minimize(
                    train_with_psqi, activity_1min, uid,
                    dim_models, feature_cols, top_k=3,
                )

                if recs:
                    top = recs[0]
                    out["psqi_predicted_total"] = float(top["predicted_total"])
                    out["recommended_slot"] = {
                        "slot_hour":       int(top["slot_hour"]),
                        "predicted_dims":  {k: float(v) for k, v in top["predicted_dims"].items()},
                        "predicted_total": float(top["predicted_total"]),
                    }
                    # LLM prompt block — 정형 정보를 텍스트로 + 방향·우선순위 강조
                    slot_h = top["slot_hour"]
                    preds_text = ", ".join(f"{k}={v:.2f}" for k, v in top["predicted_dims"].items())
                    delta = (top["predicted_total"] - current["total"]) if current["total"] is not None else None
                    if delta is not None:
                        if delta < 0:
                            delta_text = f"({delta:+.2f}, *낮아짐 = 수면 질 개선*)"
                        elif delta > 0:
                            delta_text = f"({delta:+.2f}, *높아짐 = 수면 질 저하*)"
                        else:
                            delta_text = f"({delta:+.2f}, 변화 없음)"
                    else:
                        delta_text = ""
                    cur_text = (f"c1={current['c1']}, c2={current['c2']}, "
                                f"c3={current['c3']}, c4={current['c4']}, "
                                f"합산={current['total']}/12") if current["total"] is not None else "측정 데이터 부족"
                    out["prompt_block"] = (
                        f"\n\n---\n[중요: PSQI 4 차원 보강 — 옵트인 활성]\n"
                        f"\n"
                        f"**PSQI 척도 방향: 점수가 낮을수록 수면 질이 좋습니다.**\n"
                        f"  0/12 = 최상의 수면 질, 12/12 = 최악의 수면 질\n"
                        f"\n"
                        f"현재 회원 점수: {cur_text}\n"
                        f"\n"
                        f"**[운동 시간대 추천 — 이 슬롯을 plan의 *운동* 항목에 최우선 반영하세요]**\n"
                        f"  추천 운동 종료 시각: {slot_h:02d}:00–{slot_h+1:02d}:00 (4 모델 forward simulation 합산 최소화)\n"
                        f"  예측 차원별 점수: {preds_text}\n"
                        f"  예측 합산: {top['predicted_total']:.2f}/12 {delta_text}\n"
                        f"\n"
                        f"위에 다른 시간대 추천(§8.2 통계 등)이 있어도 *이 forward simulation 슬롯을 우선*하세요.\n"
                        f"plan의 운동 시작/종료 시각 표기에 이 슬롯을 사용하고, summary에 PSQI 점수 변화의 의미\n"
                        f"(낮아지면 좋아짐, 높아지면 나빠짐)를 정확히 반영하세요.\n"
                    )
        else:
            # exploration — cold-start 4주 round-robin
            label, start_h, end_h = slot_info
            out["recommended_slot"] = {
                "slot_label": label,
            ...
            out["prompt_block"] = (
                f"\n\n---\n[Cold-start 단계 안내]\n"
                f"신규 회원 4주 데이터 수집 단계. 이번 주 권장 슬롯: {label} "
                f"({start_h:02d}-{end_h:02d}시).\n"
                f"forward simulation 기반 맞춤 추천은 5주차부터 활성.\n"
            )
    except Exception as e:
        logging.warning(f"[Phase D] PSQI 통합 실패 → fallback: {e}")
...
def main(uid: str, model_path: Path, window: int) -> dict:   # §2.1: dict 반환 (메시지 + 평가 메트릭)
    ...
    # Phase D — PSQI 4 모델 + Forward Simulation 추천 (옵트인 default 0)
    _psqi_block = {
        "psqi_scores": None, "psqi_predicted_total": None,
        "recommended_slot": None, "recommendation_mode": None, "prompt_block": "",
    }
    if os.getenv("USE_PSQI_FORWARD_SIM", "0") == "1":
        _psqi_block = _compute_psqi_block(uid, train_master)
        prompt = prompt + _psqi_block["prompt_block"]
    ...
    # §2.1 + §8.5 + Phase D dict 반환: 메시지·평가 메트릭·모델/프롬프트 메타·권장 슬롯·PSQI
    return {
        "message":               message,
        "rmse_test":             rmse_test,
        "mae_test":              mae_test,
        ...
        # Phase D — PSQI 4 모델 결과 (옵트인 미사용 시 모두 None)
        "psqi_scores":           json.dumps(_psqi_block["psqi_scores"], ensure_ascii=False) if _psqi_block["psqi_scores"] else None,
        "psqi_predicted_total":  _psqi_block["psqi_predicted_total"],
        "recommended_slot_psqi": json.dumps(_psqi_block["recommended_slot"], ensure_ascii=False) if _psqi_block["recommended_slot"] else None,
        "recommendation_mode":   _psqi_block["recommendation_mode"],
    }
